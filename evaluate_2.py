...
METHODS = ['Max Reward', 'Best Direction','Least Interfered','Strongest Neighbor']
    ...
    for i in range(n_layouts):
        ...
        for agent in agents:
            results.append([agent.flow.bottleneck_rate, len(agent.flow.get_links()),
                            ...
                            agent.flow.tot_power, agent.adhocnet.used_bands,agent.meanCap])
        for agent in agents:
            agent.reset()
    results = np.array(results); assert np.shape(results)==(n_layouts*adhocnet.n_flows,7 )
    results = np.reshape(results, (n_layouts, adhocnet.n_flows, 7))
    return results
    ...
    if (not args.visualize) and (not args.step):
        ...
        for method in METHODS:
            ...
            for i, (method, results) in enumerate(all_results.items()):
                ...
                reached_packets = np.nanmean(packets_reach, axis=1) / num_packets * 100  # percentage of reach packets.
                sumrates  = np.nansum(rates, axis=1)
                maxrates, minrates = np.max(rates, axis=1), np.min(rates, axis=1)
                print(
                    "[{}] Avg SumRate: {:.3g}Mbps; Avg MinRate: {:.3g}Mbps; Avg MaxRate: {:.3g}Mbps; Avg Rate: {:.3g}Mbps; Avg # links per flow: {:.1f}; Avg # reprobes per flow: {:.2g}".format(
